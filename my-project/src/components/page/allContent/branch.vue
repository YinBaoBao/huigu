...
export default{
  ...
  created:function(){
	...
	methods:{
	    getUsers(){				//查询初始数据
	      this.$http.get(this.$store.state.api+'/bankbranch/',{params:{pageNum:this.pageIndex,pageSize:this.pageSize}},{emulateJSON: true})
	      .then((res)=>{
    ...
    getbank(){
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post(this.$store.state.api+'/bankbranch/insert',{id:this.form.id,bankId:this.form.headoffice,provinceId:this.form.province,cityId:this.form.city,branchName:this.form.Addbranch})
          .then((res)=>{
            this.getUsers();
          });
    ...
    clearUp(){
     ...
     onSubmitU:function(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
            this.$http.post(this.$store.state.api+'/bankbranch/update',{id:this.form.updateID,provinceId:this.form.province,cityId:this.form.city,bankId:this.form.headoffice,branchName:this.form.Addbranch})
            .then((res)=>{
              this.getUsers();
            })
    ...
    fetchData: function(){
      this.$http.get(this.$store.state.api+'/bankbranch/',{params:{pageNum:this.pageIndex,pageSize:this.pageSize}},{emulateJSON: true})//url 发送的值
      .then((res)=>{
        this.tableData = res.body.dataList; 
      })
    },
    ...
    mainSearch:function(){    //查询
      this.fullscreenLoading = true;
        setTimeout(() => {
          this.fullscreenLoading = false;
          this.$http.get(this.$store.state.api+'/bankbranch/',{params:{pageNum:1,pageSize:this.pageSize,bankBranchName:this.mainSearchinput}},{emulateJSON: true})
          .then((res)=>{
          this.tableData = res.body.dataList;
          })
        }, 1000);
