  ...
  <div id="userM">
    ...
    <div class="mainSearch">
      ...
      <el-select class="mainSearchFloat paddingButton" v-model="departmenta" placeholder="请选择部门">
        <el-option v-for="item in departmentaa" :key="item.id" :label="item.departmentName" :value="item.id"></el-option>
      </el-select>
    ...
    <div class="mainNavaddB" v-show="mainAddShow">
      ...
      <hr color="#E5E9F2" width="90%">
          ...
          <el-form-item class="floatLeft" id="selectContent" label="部门">
            <el-select class="main-add-input-B" v-model="departmentId" placeholder="部门">
                <el-option v-for="item in departmentIds" :key="item.id" :label="item.departmentName" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
          <div class="clear"></div>
...
export default{
  ...
  methods:{
    getUsers:function(){
       this.$http.get(this.$store.state.api+'/user/pages',{params:{pageNum:this.pageIndex,pageSize:this.pageSize,type:1,}}, {emulateJSON: true}) /*type 1 公司*/
      .then((res)=>{
    ...
    onSubmit:function(formName){  //获取输入框的内容向接口发送
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post(this.$store.state.api+'/user/insert',{loginName:this.form.loginName,password:this.form.password,phoneNumber:this.form.phoneNumber,realName:this.form.realName,telNumber:this.form.telNumber,channelBankId:this.channelBankId,channelId:this.channelId,departmentId:this.departmentId,description:this.form.description,email:this.form.email,})
          .then((res)=>{
            this.getUsers();
            if(res.body.message){
    ...
    onSubmitU:function(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post(this.$store.state.api+'/user/update',{id:this.form.id,loginName:this.form.loginName,password:this.form.password,phoneNumber:this.form.phoneNumber,realName:this.form.realName,telNumber:this.telNumber,channelBankId:this.channelBankId,channelId:this.channelId,departmentId:this.departmentId,description:this.form.description,email:this.form.email,})
          .then((res)=>{
            this.getUsers();
          });
    ...
    fetchData: function(){
      this.$http.get(this.$store.state.api+'/user/pages',{params:{pageNum:this.pageIndex,pageSize:this.pageSize,type:1,}},{emulateJSON: true})
      .then((res)=>{
        this.tableData = res.body.dataList;
      })
    },
    mainSearch:function(){    //查询
      this.fullscreenLoading = true;
        setTimeout(() => {
          this.fullscreenLoading = false;
          this.$http.get(this.$store.state.api+'/user/pages',{params:{pageNum:1,pageSize:this.pageSize,loginName:this.mainSearchinput,channelId:this.channelIda,departmentId:this.departmenta,type:1,}},{emulateJSON: true})
          .then((res)=>{
          this.tableData = res.body.dataList;
          this.channelIda='';
          this.departmenta='';
